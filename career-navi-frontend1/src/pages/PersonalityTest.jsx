...
function PersonalityTest() {
    ...
    const [isLoading, setIsLoading] = useState(false);  // 빠른 클릭 방지
    ...
    const handleAnswer = async (answerScore) => {
        ...
        try {
            await axios.post('https://career-navi-backend.onrender.com/api/test/answer', {
                sessionId,
                questionNo: qNo,
            ...
            if (current < questions.length - 1) {
                ...
                const result = await axios.post('https://career-navi-backend.onrender.com/api/test/submit', {
                    sessionId,
                    userInfo: {
                        name: '황지영',
                        gender: '100324',
        ...
        <div className="flex min-h-screen font-sans">
            ...
            <div className="flex-1 relative bg-gradient-to-br from-indigo-50 to-purple-100 pt-24 px-6 pb-20">
                ...
                <header className="fixed top-0 left-64 right-0 h-20 px-8 bg-white/60 backdrop-blur-lg border-b border-white/20 shadow z-50 grid grid-cols-[auto_1fr_auto] items-center">
                    <h1 className="text-xl font-bold text-gray-800 whitespace-nowrap">성향 검사</h1>
                    ...
                    <nav className="flex items-center gap-6 text-sm font-medium justify-self-end">
                        ...
                        <Link to="/chat" className="px-4 py-2 rounded-full bg-purple-600 text-white hover:bg-purple-700">
                            💬 AI커비와 상담하기
                        </Link>
                    </nav>
                </header>
                            ...
                            return (
                                answerText && (
                                    <button
                                        ...
                                        } bg-purple-500 text-white px-4 py-2 rounded-xl transition`}
                                    >
                                        {answerText}
                                    </button>
                                )
