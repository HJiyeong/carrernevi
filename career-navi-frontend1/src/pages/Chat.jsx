...
function Chat() {
  ...
  useEffect(() => {
    ...
    const fetchHistory = async () => {
      const { data } = await axios.get("https://career-navi-backend.onrender.com/api/chat/messages", {
        params: { sessionId },
      });
      setMessages(
          data.map((m) => ({
  ...
  const fetchSessions = async () => {
    const { data } = await axios.get("https://career-navi-backend.onrender.com/api/chat/sessions");
    setSessions(data);
  };
        ...
        >
          ...
          <div className="h-14 bg-white z-50 border-b border-gray-200 px-6 flex items-center justify-between shadow sticky top-0">
            <button
                ...
                className="text-sm bg-purple-100 hover:bg-purple-200 text-purple-700 font-semibold px-3 py-1 rounded-full"
            >
              채팅 기록
            </button>
